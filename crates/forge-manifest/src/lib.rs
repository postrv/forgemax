...
pub fn server_entry_from_tools(
    ...
    for tool in tools {
        let (category_name, _tool_name) = split_tool_name(&tool.name);
        categories
...
mod tests {
    ...
    fn manifest_built_from_tools_list_response() {
        let tools = vec![
            McpTool {
                ...
                input_schema: Some(serde_json::json!({"type": "object", "properties": {"file": {"type": "string"}}})),
            },
            McpTool {
                name: "ast.query".into(),
        ...
        assert_eq!(entry.categories["ast"].tools[0].description, "Parse a source file");
    ...
    fn manifest_built_from_multiple_servers() {
        ...
        let tools_b = vec![
            McpTool { name: "tool3".into(), description: None, input_schema: None },
        ];
        let tools_c = vec![
            McpTool { name: "x.tool4".into(), description: None, input_schema: None },
            McpTool { name: "x.tool5".into(), description: None, input_schema: None },
            McpTool { name: "y.tool6".into(), description: None, input_schema: None },
        ];
    ...
    fn manifest_categorises_tools_by_prefix() {
        let tools = vec![
            McpTool { name: "ast.parse".into(), description: None, input_schema: None },
            McpTool { name: "ast.query".into(), description: None, input_schema: None },
            McpTool { name: "symbols.find".into(), description: None, input_schema: None },
        ];
    ...
    fn manifest_handles_flat_tool_names() {
        let tools = vec![
            McpTool { name: "grep".into(), description: None, input_schema: None },
            McpTool { name: "find".into(), description: None, input_schema: None },
            McpTool { name: "replace".into(), description: None, input_schema: None },
        ];
        ...
        let tool_names: Vec<&str> = entry.categories["general"].tools.iter().map(|t| t.name.as_str()).collect();
    ...
    fn manifest_from_tools_serializes_consistently() {
        let tools = vec![
            McpTool { name: "b.tool2".into(), description: None, input_schema: None },
            McpTool { name: "a.tool1".into(), description: None, input_schema: None },
            McpTool { name: "b.tool3".into(), description: None, input_schema: None },
        ];
    ...
    fn manifest_carries_input_schema_through() {
        ...
        let entry = server_entry_from_tools("test", "test", tools);
        assert_eq!(entry.categories["search"].tools[0].input_schema, Some(schema));
    }
