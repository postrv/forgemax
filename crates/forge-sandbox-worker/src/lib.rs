...
//!
//! This library is the implementation behind the `forgemax-worker` binary.
//! The parent process ([`forge_sandbox::host::SandboxHost`]) spawns that
//! binary, which calls [`run`]. The worker receives code and configuration
//! over stdin, executes it in a V8 isolate, and sends results back over
//! stdout. Tool calls are proxied through the parent via the IPC protocol.
//!
...
/// Run the worker event loop until the parent closes stdin or a fatal IPC error.
