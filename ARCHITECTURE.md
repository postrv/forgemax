...
- [ ] Set `max_resource_size_mb` to limit resource content sizes (default: 64 MB; tighten for memory-constrained deployments)
...
- IPC message limits: Configurable per-deployment, defaults to 65 MB (sized for a 64 MB resource payload + 1 MB envelope; tighten for memory-constrained deployments)
